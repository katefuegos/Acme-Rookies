...
public class ApplicationRookieController extends AbstractController {
	...
	public ModelAndView list(final RedirectAttributes redirectAttrs) {
		...
		try {
			...
			final Collection<Application> applicationsPending = this.applicationService.findPendingByRookieId(rookieId);
			final Collection<Application> applicationsRejected = this.applicationService.findRejectedByRookieId(rookieId);
			final Collection<Application> applicationsAccepted = this.applicationService.findAcceptedByRookieId(rookieId);
			final Collection<Application> applicationsSubmitted = this.applicationService.findSubmittedByRookieId(rookieId);

			//			final Collection<Application> applications = this.applicationService.findByRookieId(rookieId);
			//
	...
	public ModelAndView create(final RedirectAttributes redirectAttrs) {
		...
		try {
			...
			rookie = this.rookieService.findRookieByUseraccount(LoginService.getPrincipal());
			Assert.notNull(rookie);
			curriculas = this.curriculaService.findNoCopies(rookie.getId());
	...
	public ModelAndView edit(final int applicationId, final RedirectAttributes redirectAttrs) {
		...
		try {
			...
			else if (!application.getRookie().equals(rookie))
	...
	protected ModelAndView createModelAndView(final ApplicationForm applicationForm, final String message) {
		...
		final Collection<Curricula> curriculas = this.curriculaService.findNoCopies(this.rookieService.findRookieByUseraccount(LoginService.getPrincipal()).getId());
		final Collection<Position> positions = this.positionService.findFinalMode();

		result = new ModelAndView("application/create");

		result.addObject("message", message);
		result.addObject("requestURI", "application/rookie/create.do");
	...
	protected ModelAndView editModelAndView(final ApplicationForm applicationForm, final String message) {
		...
		result.addObject("message", message);
		result.addObject("requestURI", "application/rookie/edit.do?applicationId=" + applicationForm.getId());
		result.addObject("applicationForm", applicationForm);
		result.addObject("isRead", false);
		result.addObject("banner", this.configurationService.findAll().iterator().next().getBanner());
		result.addObject("systemName", this.configurationService.findAll().iterator().next().getSystemName());
	...
	protected ModelAndView displayModelAndView(final ApplicationForm2 applicationForm, final String message) {
		...
		result.addObject("message", message);
		result.addObject("requestURI", "application/rookie/display.do?applicationId=" + applicationForm.getId());
		result.addObject("applicationForm", applicationForm);
